    ...
    <div class="py-3">
        <main class="h-full overflow-y-auto">
            <div class="container mx-auto grid">
                <h6 class="mb-4 font-semibold dark:text-white text-gray-900">
                    <?= _CONSOLE_FOR ?> <?= $service['configuration']['serviceId']['taskName'] . "-" . $service['configuration']['serviceId']['taskServiceId'] ?></h6>
                    ...
                    <div class="w-full">
                        <div class="coding inverse-toggle px-5 pt-4 shadow-lg text-gray-100 dark:bg-gray-800 bg-white pb-6 pt-4 rounded-lg leading-normal overflow-hidden">
                            ...
                            <button type="button"
                                ...
                                <?= _CONSOLE_SEND_COMMAND ?>
                            </button>
                        </div>
                    </div>
                    <!-- Create text output node -->
                    <div class="w-full">
                        <div class="coding inverse-toggle px-5 pt-4 shadow-lg text-gray-100 h-96 pb-6 pt-4 rounded-lg leading-normal overflow-y-auto" id="scroll">
                            <?php
                            $curl = curl_init();
...
<script>
    $(document).ready(() => {
        let socketUrl = "<?= "ws://" . urlHelper::getConfig()['cloudnet']['ip'] . ":" . urlHelper::getConfig()['cloudnet']['port'] . "/websocket/console"; ?>?auth=<?= $_SESSION['cn3-wi-access_token'] ?>&serviceId=<?= $service_name ?>";
    ...
    function showMessage(messageHTML) {
        $('#socket_event')?.append(messageHTML);
        scrollDown()
    }
